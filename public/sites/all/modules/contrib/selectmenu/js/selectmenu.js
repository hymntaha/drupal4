  ...
  attach: function(context, settings) {
    (function ($) {
      ...
      $('select').each(function(context){
        ...
        if(apply_selectmenu && $.browser.msie && $.browser.version < 9){
          for(var i = 0;i < Drupal.settings.selectmenu.ie_form_id_exceptions.length; i++){
            if( form.attr('id').indexOf(Drupal.settings.selectmenu.ie_form_id_exceptions[i]) >= 0){
              apply_selectmenu = false;
              break;
            }
          }
        ...
        if (apply_selectmenu  && !in_array(form.attr('id'), Drupal.settings.selectmenu.form_id_exceptions)) {
